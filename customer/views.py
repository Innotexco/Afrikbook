    ...
    try:
        ...
        for item in cancel_invioice:
            ...
            if inital_invioice.invoice_state == "Supplied":
                ...
                CreateOutletStockinLog(db, item.invoice_date, item.invoiceID, item.order_ID, item.customer_name, " ", outlet, item.Gdescription, item.item_name, item.item_description, item.qty, item.token_id, item.Userlogin,  item.itemcode, item.unit_p, "")
