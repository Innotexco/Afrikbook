...
def BalanceSheet(request):
    ...
    def _d(value):
        try:
            return D(str(value or 0)).quantize(D('0.01'))
        except Exception:
            return D('0.00')

    def chart_sum(**filters):
        qs = chart_of_account.objects.using(db).filter(**filters)
        return _d(qs.aggregate(t=DSum('actual_balance'))['t'])

    def chart_sum_q(q_obj):
        qs = chart_of_account.objects.using(db).filter(q_obj)
        return _d(qs.aggregate(t=DSum('actual_balance'))['t'])
    ...
    def series_sum(model, q_obj):
        try:
            ...
            return _d(qs.aggregate(t=DSum('amount'))['t'])
        except Exception:
            return D('0.00')
    ...
    if use_series:
        cash_sum = series_sum(
            Assets_account,
            Q(account_type__iexact='Cash') | Q(account_type__icontains='Bank')
        )
        ar_sum = series_sum(
            Assets_account,
            Q(account_type__icontains='Receivable')
            | Q(account_bankname__icontains='Receivable')
            | Q(account_bankname__icontains='Debtors')
        )
        prepaid_sum = series_sum(
            Expenses_account,
            Q(account_bankname__icontains='prepaid') | Q(account_type__icontains='prepaid')
        )
        ppe_sum = series_sum(
            Assets_account,
            Q(account_bankname__icontains='Property')
            | Q(account_bankname__icontains='plant')
            | Q(account_bankname__icontains='equipment')
            | Q(account_type__icontains='Fixed')
            | Q(account_type__icontains='PPE')
        )
        ...
        ap_sum = series_sum(
            Liability_account,
            (
                Q(account_type__icontains='Payable')
                | Q(account_bankname__icontains='Payable')
                | Q(account_bankname__icontains='Purchase')
            )
            & ~Q(account_bankname__icontains='tax')
            & ~Q(account_type__iexact='Tax')
        )
        tax_payable_sum = series_sum(
            Liability_account,
            Q(account_bankname__icontains='tax') | Q(account_type__iexact='Tax')
        )
        long_term_sum = series_sum(
            Liability_account,
            Q(status__icontains='Long')
            | Q(account_type__icontains='Long')
            | Q(account_bankname__icontains='Loan')
        )
        ...
        capital_sum = chart_sum_q(
            Q(series_name__iexact='Equity')
            & (Q(account_bankname__icontains='capital') | Q(account_type__icontains='capital'))
        )
        retained_sum = chart_sum_q(
            Q(series_name__iexact='Equity')
            & (Q(account_bankname__icontains='Retained') | Q(account_type__icontains='Retained'))
        )
        ...
        if other_equity < 0:
            other_equity = D('0.00')
        retained_sum = retained_sum + other_equity
        ...
        tax_payable_sum = chart_sum_q(
            Q(series_name__iexact='Liability')
            & (Q(account_bankname__icontains='tax') | Q(account_type__iexact='Tax'))
        )
        long_term_sum = chart_sum_q(
            Q(series_name__iexact='Liability')
            & (
                Q(status__icontains='Long')
                | Q(account_type__icontains='Long')
    ...
    if not use_series:
        ...
        classified_assets = cash_sum + ar_sum + inventory_sum + prepaid_sum + ppe_sum
        other_current_assets = all_assets - classified_assets
        if other_current_assets < 0:
            other_current_assets = D('0.00')

    Total_Current_Assets = cash_sum + ar_sum + inventory_sum + prepaid_sum + other_current_assets
    ...
    context = {
        ...
        'Prepaid_Expenses_Sum': prepaid_sum,
        ...
        'acctpayableSum': ap_sum,
        'Income_Tax_Payables_Sum': tax_payable_sum,
        'Other_Current_Liabilities': other_liab_sum,
        'Total_Current_Liabilities': Total_Current_Liabilities,

        'Long_term_debit': long_term_sum,
        'Other_Sum': D('0.00'),
        'Total_Non_Current_Liabilities': Total_Non_Current_Liabilities,
        'Total_Liabilities': Total_Liabilities,

        'capital_Investment': capital_sum,
        'Retained_Earning': retained_sum,
        'Total_Owners_equity': Total_Owners_equity,
...
def OutletStockLevelReport(request):
    ...
    for log in latest:
        if log.item_code not in seen:
            ...
            default_data.append({
                'item':     log.item,
                'qtyLevel': currentStockLevel_Outlet(db, log.item_code, log.outlet),  # pass outlet
                'date':     str(log.datetx),
...
def levelReportOutlet(
    ...
    try:
        ...
        for data in report_qs:
            ...
            if daily == 'daily':
                period = strDate
                period_date_range         = Q(datetx=strDate)
                period_date_range_cusInv  = Q(invoice_date=strDate)
            elif daily == 'quaterly':
                period = quaterLog[count] if count < len(quaterLog) else quaterLog[-1]
                if count < len(quaterLog) - 1:
                    count += 1
                period_date_range        = Q(datetx__year=year) & Q(datetx__month=str(period))
                period_date_range_cusInv = Q(invoice_date__year=year) & Q(invoice_date__month=str(period))
            elif daily == 'monthly':
                period = month
                period_date_range        = select_date_range
                period_date_range_cusInv = select_date_range_for_cusInv
            else:  # yearly
                period = year
                period_date_range        = select_date_range
                period_date_range_cusInv = select_date_range_for_cusInv
            ...
            if period not in period_log:
                ...
                outlet_log_qs = CreateOutletStockInLog.objects.using(db).filter(
                    ~Q(outlet=None) & period_date_range
                )
                if outlet_filter and outlet_filter not in ('', '_ _Choose Outlet_ _'):
                    outlet_log_qs = outlet_log_qs.filter(outlet=outlet_filter)
                if item_filter and item_filter not in ('', '_ _Choose Item_ _'):
                ...
                wh_qs = CreateStockInLog.objects.using(db).filter(period_date_range)
                if outlet_filter and outlet_filter not in ('', '_ _Choose Outlet_ _'):
                    wh_qs = wh_qs.filter(warehouse=outlet_filter)
                if item_filter and item_filter not in ('', '_ _Choose Item_ _'):
                    wh_qs = wh_qs.filter(item_code=item_filter)
                for row in wh_qs:
                    all_qty_sent_from_outlet += float(row.quantity)

                oo_qs = CreateOutletStockInLog.objects.using(db).filter(period_date_range)
                if outlet_filter and outlet_filter not in ('', '_ _Choose Outlet_ _'):
                    oo_qs = oo_qs.filter(warehouse=outlet_filter)
                if item_filter and item_filter not in ('', '_ _Choose Item_ _'):
                    oo_qs = oo_qs.filter(item_code=item_filter)
                for row in oo_qs:
                    all_qty_sent_within_outlet_table += float(row.quantity)
...
def WarehouseStockLevelReport(request):
    ...
    if request.method == 'GET':
        ...
        if any([fromdate, todate, Yearly, monthly, Quaterly, item_filter, warehouse_filter]):
    ...
    try:
        ...
        for data in report:
            ...
            if period not in period_log:
                ...
                inflow_qs = CreateStockInLog.objects.using(db).filter(period_date_range)
                if warehouse_filter:
                ...
                outflow2_qs = CreateStockInLog.objects.using(db).filter(
                    Q(warehouse__icontains='warehouse') & period_date_range
                )
                if warehouse_filter:
                ...
                total_qty = all_qty_sent_to_warehouse - (
                    all_qty_sent_from_warehouse +
                    all_qty_sent_within_warehouse
                )
                qtyLog.append({
                    'qty':  total_qty,
                    'item': data.item,
...
def currentStockLevel(db, item_):
    all_qty_sent_to_store = (
        CreateStockInLog.objects.using(db)
        .filter(item_code=item_)
        .aggregate(total=Sum('quantity'))['total'] or 0.00
    )
    all_qty_sent_from_store = (
        CreateStockInLog.objects.using(db)
        .filter(Q(warehouse__icontains='warehouse') & Q(item_code=item_))
        .aggregate(total=Sum('quantity'))['total'] or 0.00
    )
    ...
    return float(all_qty_sent_to_store) - (
        float(all_qty_sent_from_store) +
        float(all_qty_sent_within_store)
    )
